...
class TestOperator:
    ...
    def test_create_operator_with_name(self, operator):
        """The operator must be created with name"""
        assert operator.id == 1
        assert operator.name == "Test name"

    def test_operator_default_values(self, operator):
