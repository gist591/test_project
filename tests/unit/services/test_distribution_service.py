...
class TestFidnOrCreateLead:
    ...
    def test_returns_existing_lead_when_found(self, service, lead_repo) -> None:
        """If lead is exist - return him, don't create new"""
        existing = lead_repo.add(Lead(id=None, external_id="tg_123", name="Test name"))

        result = service.find_or_create_lead(external_id="tg_123")
    ...
    def test_creates_new_lead_when_not_found(
        ...
        result = service.find_or_create_lead(external_id="tg_456", name="Test name2")
...
class TestSelectOperator:
    ...
    def test_return_single_operator(
        ...
        operator = operator_repo.add(Operator(id=None, name="Test name", max_load=10))
    ...
    def test_selects_by_weight_distribution(
        ...
        op1 = operator_repo.add(Operator(id=None, name="Test name1", max_load=100))
        op2 = operator_repo.add(Operator(id=None, name="Test name2", max_load=100))
        operator_source_repo.add(
            OperatorSource(operator_id=op1.id, source_id=1, weight=10)
        )
        operator_source_repo.add(
            OperatorSource(operator_id=op2.id, source_id=1, weight=30)
        )
    ...
    def test_skips_operator_at_capactity(
        self, service, operator_repo, operator_source_repo, contact_repo
    ) -> None:
        op1 = operator_repo.add(Operator(id=None, name="Op1", max_load=1))
        op2 = operator_repo.add(Operator(id=None, name="Op2", max_load=10))
        operator_source_repo.add(
            OperatorSource(operator_id=op1.id, source_id=1, weight=50)
        )
        operator_source_repo.add(
            OperatorSource(operator_id=op2.id, source_id=1, weight=50)
        )
    ...
    def test_skips_inactive_opeartor(
        self, service, operator_repo, operator_source_repo
    ) -> None:
        """Inactive operator skips"""
        op1 = operator_repo.add(
            Operator(id=None, name="Op1", is_active=False, max_load=10)
        )
        op2 = operator_repo.add(
            Operator(id=None, name="Op2", is_active=True, max_load=10)
        )
        operator_source_repo.add(
            OperatorSource(operator_id=op1.id, source_id=1, weight=50)
        )
...
class TestCreateContact:
    ...
    def test_creates_contact_with_operator(
        self, service, operator_repo, source_repo, operator_source_repo, contact_repo
    ) -> None:
        ...
        operator_source_repo.add(
            OperatorSource(operator_id=operator.id, source_id=source.id, weight=10)
        )
    ...
    def test_creates_lead_if_not_exists(
        self, service, operator_repo, source_repo, operator_source_repo, lead_repo
    ) -> None:
    ...
    def test_uses_existing_lead(
        ...
        existing_lead = lead_repo.add(
            Lead(id=None, external_id="tg_exists", name="Старый")
        )
        operator = operator_repo.add(Operator(id=None, name="Иван", max_load=10))
        source = source_repo.add(Source(id=None, name="telegram"))
        operator_source_repo.add(
        ...
        result = service.create_contact(
            external_lead_id="tg_exists", source_id=source.id
        )

        assert result.lead_id == existing_lead.id
        assert len(lead_repo.get_all()) == 1

    def test_creates_unassigned_when_no_operators(
        self, service, source_repo, contact_repo
    ):
