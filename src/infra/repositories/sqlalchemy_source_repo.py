...
class SQLAlchemySourceRepository(SourceRepository):
    ...
    def get_all(self) -> List[Source]:
        return [SourceMapper.to_domain(m) for m in self._session.query(SourceModel).all()]

    def update(self, entity: Source) -> Source:
        model = self._session.query(SourceModel).filter(SourceModel.id == entity.id).first()
        if model:
