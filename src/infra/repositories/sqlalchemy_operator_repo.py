...
from src.infra.db.models import ContactModel, OperatorModel, OperatorSourceModel
...
class SQLAlchemyOperatorRepository(OperatorRepository):
    ...
    def get_by_id(self, id: int) -> Operator | None:
        model = (
            self._session.query(OperatorModel).filter(OperatorModel.id == id).first()
        )
        if not model:
            return None
    ...
    def get_all(self) -> list[Operator]:
        ...
        return [
            OperatorMapper.to_domain(m, self._count_active_contacts(m.id))
            for m in models
        ]
    ...
    def update(self, entity: Operator) -> Operator:
        model = (
            ...
            .filter(OperatorModel.id == entity.id)
            .first()
        )
        if model:
    ...
    def get_available_for_source(self, source_id: int) -> list[tuple[Operator, int]]:
        ...
        results = (
            self._session.query(
                ...
                func.coalesce(active_count_subq.c.active_count, 0).label(
                    "current_load"
                ),
            )
            .join(
                OperatorSourceModel, OperatorModel.id == OperatorSourceModel.operator_id
            )
            .outerjoin(
                active_count_subq, OperatorModel.id == active_count_subq.c.operator_id
            )
            .filter(
                OperatorSourceModel.source_id == source_id,
                OperatorModel.is_active,
            )
            .all()
        )
