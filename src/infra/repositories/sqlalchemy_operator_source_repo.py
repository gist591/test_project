...
class SQLAlchemyOperatorSourceRepository(OperatorSourceRepository):
    ...
    def get_all(self) -> List[OperatorSource]:
        return [OperatorSourceMapper.to_domain(m) for m in self._session.query(OperatorSourceModel).all()]
    ...
    def get_by_source_id(self, source_id: int) -> List[OperatorSource]:
        models = self._session.query(OperatorSourceModel).filter(OperatorSourceModel.source_id == source_id).all()
        return [OperatorSourceMapper.to_domain(m) for m in models]

    def get_by_operator_id(self, operator_id: int) -> List[OperatorSource]:
        models = self._session.query(OperatorSourceModel).filter(OperatorSourceModel.operator_id == operator_id).all()
        return [OperatorSourceMapper.to_domain(m) for m in models]

    def delete(self, operator_id: int, source_id: int) -> bool:
        result = (
            self._session.query(OperatorSourceModel)
            .filter(OperatorSourceModel.operator_id == operator_id, OperatorSourceModel.source_id == source_id)
            .delete()
        )
        self._session.commit()
        return result > 0
